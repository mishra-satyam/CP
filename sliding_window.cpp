...
struct sliding_max {
	...
	void push(T v) {
		frontv.push_back(v);
		if (frontmx.size()) frontmx.push_back(/*operator*/(frontmx.back(), v)); // combine
	...
	void pop() {
		if (!backv.size()) {
			while (frontv.size()) {
				...
				if (backmx.size()) backmx.push_back(/*operator*/(backmx.back(), frontv.back())); // combine
	...
	T query() {
		if (frontmx.size() && backmx.size()) return /*operator*/(frontmx.back(), backmx.back()); // combine
